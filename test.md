...
3. It fills `admin` / `admin` automatically (These are the actual superuser credentials)
4. Click **Sign In**
5. **Expected:** Redirected to `/dashboard` with full admin sidebar. Auth state is persisted via JWT in localStorage.

### 2.2 Employee Login
1. Click the **"👤 Employee"** quick-login button
2. It fills `employee` / `1234` automatically (These are also auto-created by the backend)
3. Click **Sign In**
...
2. **Expected:** Red error alert appears ("Invalid credentials. Please try again."). API returns 401 Unauthorized.
...
### 5.5 ✨ New Order + Payment Flow (Full Test)
...
## 11. Project Integration Status

| Component                      | Status   | Notes                                       |
|-------------------------------|----------|---------------------------------------------|
| Authentication                | ✅ API   | JWT tokens with secure role-based access    |
| Data Persistence              | ✅ DB    | All modules saved to SQLite/PostgreSQL      |
| Payment Simulation            | ✅ API   | Orders saved with payment method metadata    |
| Photo Upload                  | ✅ API   | Supports local file upload (Base64) & URLs  |
| Real-time Stats               | ✅ API   | Dashboard charts reflect actual DB data     |
